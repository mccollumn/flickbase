...
import { formValues, FormValueProps, validation } from "./validationSchema";
...
const EditArticle = () => {
  ...
  const [formData, setFormData] = React.useState<FormValueProps>(formValues);
    ...
    dispatch(getAdminArticle(articleId))
      ...
      .then((response) => {
        if (response.category) {
          if (typeof response.category === "object" && response.category._id) {
            response.category = response.category._id;
          }
        }
        setFormData(response as FormValueProps);
